...
let saveImageOnIpfs = (reader) => {
  return new Promise(function(resolve, reject) {
    ...
    ipfs.add(buffer).then((response) => {
      console.log("saveImageOnIpfs"+response)
...
//    });
  ...
  instantiateContract = () => {
    ...
    this.state.web3.eth.getAccounts((error, accounts) => {
      account = accounts[0];
      console.log(account);
      simpleStorage.at('0xc8461d2f2cfc749c570919e19b4df2b17ecd806e').then((contract) => {
  ...
  render() {
    return (<div className="App">
      ...
      }
      {
        this.state.blockChainHash
          ? <div>
              <h2>浏览器访问：{"https://ipfs.infura.io/ipfs/" + this.state.imgHash}</h2>
              <img alt="" style={{
                  width: 1600
                }} src={"https://ipfs.infura.io/ipfs/" + this.state.imgHash}/>
            </div>
          : <img alt=""/>
      }
    </div>);
